...
namespace MM_Localization {
  ...
  public class MMStringConverter : IValueConverter {
    public string convert(string s) {
      if (string.IsNullOrEmpty(s)) return "";
      string retStr = "";

      for (int i = 0, firstInLineI = 0; i < s.Length; i++) {
        if (s[i] == '{') { retStr += s[i]; continue; }

        if (s[i] == '}') { retStr += s[i]; firstInLineI = i + 1; continue; }

        if (i - firstInLineI >= 40) {
          retStr += "{}";
          firstInLineI = i;
        }

        if (s[i] != '\n') retStr += s[i];
        else { retStr += "{}"; firstInLineI = i + 1; }
        
      }
      return retStr;
    }

    public string convertForSave(string s) {
      string retStr = "";
  ...
  public class textsTextsViewModel {
    ...
    private List<txtAddresses> textAddresses = new List<txtAddresses> {
      new txtAddresses("mm.exe",       new addrPair[] {
        ...
        new addrPair(seg000+0xC2AB),      //:382D;GOES DOWN!!!
      ...
      new txtAddresses("enf1.ovr",     new addrPair[] {
        new addrPair(0x005C), // ETCHED IN GOLD, MESSAGE 3 READS:\FOR-TASKS-...
        new addrPair(0x0069), // MINOTAUR DUNG SLUSHES BENEATH YOU!
        new addrPair(0x0079), // THE ENCHANTED CHAMBER
        new addrPair(0x0089), // CLEPTOMIAN GUILD, DO NOT DISTURB!
        new addrPair(0x0099), // THE COUNCIL OF STRENGTH IS IN SESSION
        new addrPair(0x00A9), // THE UGLIES CLUB - MEMBERS ONLY!
        new addrPair(0x00B9), // SPECIMEN TANK - KEEP CLOSED
        new addrPair(0x00C9), // SPIRALLING STAIRS GO UP,TAKE THEM (Y/N)?
        new addrPair(0x00FC), // STAIRS GOING DOWN, TAKE THEM (Y/N)?
        new addrPair(0x01D2), // A SIGN ON THE DOOR READS:\
        new addrPair(0x01DC), // THE ROOM IS PAINTED FROM CEILING TO\FLOOR I...
        new addrPair(0x01E9), // A TAPESTRY ON THE WALL DEPICTS A GRAY\MINOT...
        new addrPair(0x01F6), // A TAPESTRY ON THE WALL DEPICTS\A KINGDOM RU...                     
      }),
      new txtAddresses("enf2.ovr",     new addrPair[] {
        new addrPair(0x016B), // ETCHED IN GOLD, MESSAGE 9 READS:\SUCCESSFUL...
        new addrPair(0x0178), // THE MASTER OF THIS STRONGHOLD\HAS BEEN DEFE...
        new addrPair(0x01FD), // ENTER THE MAZE OF THE MINOTAUR...\IF YOU DARE!
        new addrPair(0x022B), // STAIRS GOING UP, TAKE THEM (Y/N)?
        new addrPair(0x0060), // STANDING ATOP A PEDESTAL IN THE CENTER\OF T...
        new addrPair(0x0105), // IT COMES TO LIFE AND\SPEAKS,"CONGRATULATION...
        new addrPair(0x00AA), // DESECRATE IT (Y/N)?
      }),
      new txtAddresses("erliquin.ovr", new addrPair[] {
        new addrPair(0x01AB),         // A SIGN ABOVE THE DOOR READS:\
        new addrPair(0x021F),         // STEP UP TO THE BAR (Y/N)?
        new addrPair(0x026D),         // STAIRS GOING DOWN! TAKE THEM (Y/N)?
        new addrPair(0x01FA),         // PASSAGE OUTSIDE, EXIT (Y/N)?
        new addrPair(0x008F),         // ADJUSTING HIS SPECTACLES, AN ELDERL...
        new addrPair(0x0291,0x028F),  // "FOUR STAR FOODS"
        new addrPair(0x0299,0x0297),  // "CURRENT TRENDS IRONWORKS"
        new addrPair(0x02A1,0x029F),  // "THE INN OF ERLIQUIN"
        new addrPair(0x01BF),         // A BOISTROUS HALF-ORC PROCLAIMS,\"WE...
        new addrPair(0x01D2),         // THE CHEF SAYS,"I ONLY SERVE THE BES...
        new addrPair(0x0232),         // SEVERAL FAT CLERICS WELCOME YOU.\NE...
        new addrPair(0x0245),         // THREE HUGE ARMOR CLAD KNIGHTS BARK ...
        new addrPair(0x02B1,0x02AF),  // "TAVERN OF TALL TALES"
        new addrPair(0x02A9,0x02A7),  // "TEMPLE GAUCHE"
        new addrPair(0x02B9,0x02B7),  // "SUPERIOR TRAINING"
        new addrPair(0x00FB),         // "I HAVE NO BUSINESS WITH YOU. BEGONE!"
        new addrPair(0x014D),         // "YOU HAVE DONE WELL TO BRING ME THI...
        new addrPair(0x00C9),         // THE WIZARD AGAR SPEAKS:\\
        new addrPair(0x02F0),         // POOF!
        new addrPair(0x0359),         // TOWN GUARDS: YOU'RE UNDER ARREST!\\...
        new addrPair(0x0393),         // HIGH COURT SENTENCE: YEARS= 
        new addrPair(0x0038),         // TOWN TREASURE, STEAL (Y/N)?
      }),
      new txtAddresses("portsmit.ovr", new addrPair[] {
        new addrPair(0x0060),         // LARA, THE INKEEPERS VOLUPTUOUS\DAUG...
        new addrPair(0x0109),         // A SIGN ABOVE THE DOOR READS:\ 
        new addrPair(0x029B,0x0299),  // "MAID MARION'S MARKET" 
        new addrPair(0x02A3,0x02A1),  // "B. SMITH'S WORKSHOP" 
        new addrPair(0x02AB,0x02A9),  // "INN OF PORTSMITH" 
        new addrPair(0x011D),         // BUSILY FORGING A LONG SWORD,AN EXTR...
        new addrPair(0x0139),         // FROM BEHIND THE FRUIT STAND\A SULTR...                                  
        new addrPair(0x0183),         // STEP UP TO THE BAR (Y/N)?
        new addrPair(0x019F),         // THE ONCE MAGNIFICENT TEMPLE SHOWS\S...
        new addrPair(0x01BB),         // A TALL, MUSCULAR FEMALE CLAD IN LEA...
        new addrPair(0x026E),         // STAIRS GOING DOWN! TAKE THEM (Y/N)?
        new addrPair(0x02BB,0x02B9),  // "ZORELDA'S WATERING HOLE"
        new addrPair(0x02B3,0x02B1),  // "TEMPLE SUCCUBON"
        new addrPair(0x02C3,0x02C1),  // "AMAZONIA'S TRAINING"
        new addrPair(0x0155),         // PASSAGE OUTSIDE, EXIT (Y/N)?
        new addrPair(0x0091),         // "I AM ZAM, ASTRAL BROTHER OF ZOM AN...
        new addrPair(0x00D0),         // MY CLUE IS C-15"
        new addrPair(0x00C3),         // YOU'RE NOT THE COURIERS!"
        new addrPair(0x01D7),         // A SENSUOUS FEMALE VOICE SPEAKS FROM...
        new addrPair(0x02EB),         // NOTE: ZAM 12,2
        new addrPair(0x0397),         // MALES ARE DRAINED!
      }),
      new txtAddresses("pp1.ovr",      new addrPair[] {
        new addrPair(0x0060),         // ETCHED IN GOLD, MESSAGE 7 READS:\TH...
        new addrPair(0x006F,0x006D),  // CLUB DEAD...\THE ANTIDOTE TO HUMANI...
        new addrPair(0x0077,0x0075),  // TEMPLE OF THE OLD ORDER
        new addrPair(0x007F,0x007D),  // GLASS ROOM
        new addrPair(0x0087,0x0085),  // RUBY ROOM
        new addrPair(0x008F,0x008D),  // TWINKLE ROOM
        new addrPair(0x0097,0x0095),  // RAINBOW ROOM
        new addrPair(0x00B3),         // A PASSAGE LEADS OUTSIDE, TAKE IT (Y...
        new addrPair(0x00E6),         // STAIRS GOING DOWN, TAKE THEM (Y/N)?
        new addrPair(0x010F),         // STAIRS GOING DOWN, TAKE THEM (Y/N)?
        new addrPair(0x0138),         // STAIRS GOING DOWN, TAKE THEM (Y/N)?
        new addrPair(0x0218),         // THE ROOM IS PAINTED FROM CEILING TO...
        new addrPair(0x0261),         // THE CRYSTAL GRATE REPELS YOU!
      }),
      new txtAddresses("pp2.ovr",      new addrPair[] {
        new addrPair(0x00B8), // STAIRS GOING DOWN, TAKE THEM (Y/N)?
        new addrPair(0x00E1), // STAIRS GOING UP, TAKE THEM (Y/N)?
        new addrPair(0x01B5), // THE WALL IS PAINTED FROM CEILING TO\FLOOR I...
        new addrPair(0x0201), // WITHOUT EXPERIMENTAL SUBSTANCE\THERE IS NO ...                          
      }),
      new txtAddresses("pp3.ovr",      new addrPair[] {
        new addrPair(0x0060), // YOU HAVE BEEN SUMMONED...POOF!
        new addrPair(0x009B), // STAIRS GOING UP, TAKE THEM (Y/N)?
        new addrPair(0x00C4), // STAIRS GOING UP, TAKE THEM (Y/N)?
        new addrPair(0x00EA), // THE ROOM IS PAINTED FROM CEILING TO\FLOOR I...
      }),
      new txtAddresses("pp4.ovr",      new addrPair[] {
        new addrPair(0x0060), // DANGER! DUNGEON UNDER CONSTRUCTION.
        new addrPair(0x0070), // A LADDER GOING UP, CLIMB IT (Y/N)?
        new addrPair(0x0144), // THE WALL IS PAINTED IN A BLACK AND\WHITE CH...
      }),
      new txtAddresses("qvl1.ovr",     new addrPair[] {
        new addrPair(0x0058), // ETCHED IN GOLD, MESSAGE 1 READS:\COMPLETION...
        new addrPair(0x0068), // A GHOSTLIKE FIGURE RISES FROM THE BODY\OF O...
        new addrPair(0x007F), // "MAY THE SHADOW OF DEATH GREET YOU\WITH OPE...
        new addrPair(0x0116), // A GEM ENCRUSTED ARCHWAY READS:\LAIR OF THE ...
        new addrPair(0x0132), // AN OPENING ABOVE LEADS OUTSIDE,\CLIMB OUT (...                                                                    
        new addrPair(0x018A), // STAIRS GOING DOWN, TAKE THEM (Y/N)?
        new addrPair(0x0261), // THE FLOOR SPINS!
        new addrPair(0x027B), // THE WALL IS PAINTED FROM CEILING TO\FLOOR I...
      }),
      new txtAddresses("qvl2.ovr",     new addrPair[] {
        new addrPair(0x005D), // ETCHED IN GOLD, MESSAGE 4 READS:\OF-BE-HAS-...
        new addrPair(0x006A), // A BANNER READS:\THE LABYRINTH OF LAZZERUTH....
        new addrPair(0x0077), // INSET IN THE WALL, A STONE FACE\YELLS: "MY ...
        new addrPair(0x0084), // INSET IN THE WALL, A STONE FACE\SPEAKS: "I ...
        new addrPair(0x0091), // INSET IN THE WALL, A STONE FACE\SPEAKS: "I ...
        new addrPair(0x009E), // INSET IN THE WALL, A STONE FACE\PROCLAIMS: ...                      
        new addrPair(0x00AE), // STAIRS GOING UP, TAKE THEM (Y/N)?
        new addrPair(0x0189), // THE WALL IS PAINTED FROM CEILING TO\FLOOR I...
        new addrPair(0x0196), // A STRANGE RINGING SOUND STARTLES\THE PARTY....                   
      }),
      new txtAddresses("rwl1.ovr",     new addrPair[] {
        new addrPair(0x0060), // ETCHED IN GOLD, MESSAGE 6 READS:\BOOK-DONE....
        new addrPair(0x0070), // IN THE CENTER OF THE ROOM, A HUGE RUBY\STAN...
        new addrPair(0x00EA), // WRONG!  A BRIGHT FLASH...
        new addrPair(0x0181), // A SILVER DOOR REPELS YOU!
        new addrPair(0x0198), // A SILVER DOOR, YOUR SILVER KEY GLOWS!
        new addrPair(0x01A5), // STAIRS TO SURFACE, TAKE THEM (Y/N)?
        new addrPair(0x01FA), // STAIRS DOWN, TAKE THEM (Y/N)?
        new addrPair(0x02D2), // THE ROOM IS PAINTED IN A BLACK AND\WHITE CH...
      }),
      new txtAddresses("rwl2.ovr",     new addrPair[] {
        new addrPair(0x005D),         // ETCHED IN GOLD, MESSAGE 2 READS:\ON...
        new addrPair(0x0070),         // "I AM LORD ARCHER, THE RAVEN,\I TAK...
        new addrPair(0x01AF,0x01AD),  // GIANTS HURL BOULDERS!
        new addrPair(0x01B5),         // A BUTTON ON THE WALL, PUSH IT (Y/N)?
        new addrPair(0x01DF,0x01DD),  // SQUISH! THE PARTY IS IMPALED ON A W...
        new addrPair(0x01E7,0x01E5),  // A BOULDER SAYS:"THERE SHOULD BE 5 T...
        new addrPair(0x01EF,0x01ED),  // THE FIRST TEST
        new addrPair(0x01F7,0x01F5),  // THE SECOND TEST
        new addrPair(0x01FF,0x01FD),  // THE THIRD TEST
        new addrPair(0x0207,0x0205),  // THE FOURTH TEST
    ...
    private byte[] fontArr = {
      ...
      0x28,0x7C,0x60,0x78,0x60,0x60,0x7C,0x00, // 004 - Ё
      ...
      0x00,0x38,0x6C,0x6C,0x7C,0x6C,0x6C,0x00, // 040 - А
      0x00,0x7C,0x60,0x78,0x6C,0x6C,0x78,0x00, // 041 - Б
      0x00,0x78,0x6C,0x78,0x6C,0x6C,0x78,0x00, // 042 - В
      0x00,0x7C,0x60,0x60,0x60,0x60,0x60,0x00, // 043 - Г
      0x00,0x3C,0x6C,0x6C,0x6C,0x6C,0xFE,0x00, // 044 - Д
      0x00,0x7C,0x60,0x78,0x60,0x60,0x7C,0x00, // 045 - Е
      0x00,0xDB,0x7E,0x3C,0x7E,0xDB,0xDB,0x00, // 046 - Ж
      0x00,0x38,0x6C,0x18,0x0C,0x6C,0x38,0x00, // 047 - З
      0x00,0x66,0x66,0x6E,0x7E,0x76,0x66,0x00, // 048 - И
      0x18,0x66,0x66,0x6E,0x7E,0x76,0x66,0x00, // 049 - Й
      0x00,0x6C,0x78,0x70,0x78,0x6C,0x6C,0x00, // 04A - К
      0x00,0x1C,0x3C,0x6C,0x6C,0x6C,0x6C,0x00, // 04B - Л
      0x00,0xC6,0xEE,0xFE,0xD6,0xC6,0xC6,0x00, // 04C - М
      0x00,0x6C,0x6C,0x7C,0x6C,0x6C,0x6C,0x00, // 04D - Н
      0x00,0x38,0x6C,0x6C,0x6C,0x6C,0x38,0x00, // 04E - О
      0x00,0x7C,0x6C,0x6C,0x6C,0x6C,0x6C,0x00, // 04F - П
      0x00,0x78,0x6C,0x6C,0x6C,0x78,0x60,0x00, // 050 - Р
      0x00,0x38,0x6C,0x60,0x60,0x6C,0x38,0x00, // 051 - С
      0x00,0x78,0x30,0x30,0x30,0x30,0x30,0x00, // 052 - Т
      0x00,0x6C,0x6C,0x6C,0x3C,0x0C,0x38,0x00, // 053 - У
      0x00,0x7C,0xD6,0xD6,0xD6,0x7C,0x10,0x00, // 054 - Ф
      0x00,0x6C,0x6C,0x38,0x38,0x6C,0x6C,0x00, // 055 - Х
      0x00,0x6C,0x6C,0x6C,0x6C,0x6C,0x3E,0x02, // 056 - Ц
      0x00,0x6C,0x6C,0x6C,0x3C,0x0C,0x0C,0x00, // 057 - Ч
      0x00,0xC6,0xD6,0xD6,0xD6,0xD6,0x7E,0x00, // 058 - Ш
      0x00,0xC6,0xD6,0xD6,0xD6,0xD6,0x7F,0x01, // 059 - Щ
      0x00,0x70,0x30,0x3C,0x36,0x36,0x3C,0x00, // 05A - Ъ
      0x00,0xC3,0xC3,0xF3,0xDB,0xDB,0xF3,0x00, // 05B - Ы
      0x00,0x60,0x60,0x78,0x6C,0x6C,0x78,0x00, // 05C - Ь
      0x00,0x38,0x4C,0x1C,0x0C,0x6C,0x38,0x00, // 05D - Э
      0x00,0xCE,0xDB,0xFB,0xFB,0xDB,0xCE,0x00, // 05E - Ю
      0x00,0x3C,0x6C,0x6C,0x3C,0x6C,0x6C,0x00, // 05F - Я
    };
    ...
    private byte convertChar(char c) {
      ...
      switch (c) {
        ...
        case 'Ё': retC = (byte)'$'; break;
        //case 'Ё': retC = 0x45 + 0x20; break;
    ...
    public void SaveLocalizedFiles(string origPath, string savePath) {
      foreach (locText lt in locTexts) {
        ...
        MMStringConverter sconv = new MMStringConverter();
        foreach (text t in lt.dt)
          if (t.newAddrInSeg != 0) {
            t.localizedText = sconv.convertForSave(t.localizedText);
    ...
    private void ExportExcelBinding_Executed(object sender, 
      ...
      for (int i = 0; i < textsViewModel.lstFNames.Length; i++) {
        ...
        for (int j = 0; j < lt.dt.Count; j++) {
          xlWorkSheet.Cells[j + 1, 1] = sconv.convert(lt.dt[j].originalText);
          xlWorkSheet.Cells[j + 1, 2] = string.Format("=LEN(A{0})", j + 1);
          xlWorkSheet.Cells[j + 1, 3] = sconv.convert(lt.dt[j].localizedText);
